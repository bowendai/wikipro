...
function html_secedit($text, $show = true) {
    ...
    return preg_replace_callback($regexp, 'html_secedit_button', $text);
}
...
function html_secedit_button($matches) {
    ...
    return trigger_event('HTML_SECEDIT_BUTTON', $data, 'html_secedit_get_button');
}
...
function html_secedit_get_button($data) {
    ...
    return "<div class='secedit editbutton_" . $data['target'] .
            " editbutton_" . $secid . "'>" .
            html_btn('secedit', $ID, '', array_merge(array('do' => 'edit',
                'rev' => $INFO['lastmod'],
                'summary' => '[' . $name . '] '), $data), 'post', $name) . '</div>';
}
...
function html_search() {
    ...
    $intro = str_replace(
            array('@QUERY@', '@SEARCH@', '@CREATEPAGEINFO@'), array(hsc(rawurlencode($QUERY)), hsc($QUERY), $pagecreateinfo), $intro
    );
    echo $intro;
    flush();
...
function html_revisions_dai_define($first = 0, $revisions = [], $title = '', $cur_version = '', $count, $media_id = false) {
    ...
    foreach ($revisions as $rev) {
        ...
        if ($exists) {
            ...
            if (!$media_id)
                $href = wl($id, "version=$rev,do=diff", false, '&');
            else
                $href = media_managerURL(array('image' => $id, 'rev' => $rev, 'mediado' => 'diff'), '&');
...
function html_recent($first = 0, $show_changes = 'both') {
    ...
    if ($conf['mediarevisions']) {
        ...
        $form->addElement(form_makeListboxField(
            ...
            'both' => $lang['both_changes']), $show_changes, $lang['changes_type'], '', '', array('class' => 'quickselect')));
    ...
    if (!$text) {
        ...
        $form->addElement(
            ...
            'inline' => $lang['diff_inline']
                        ), $type, $lang['diff_type'], '', '', array('class' => 'quickselect')
                )
        );
...
function html_diff_dai_define($versions, $diffInfo, $text = '', $intro = true, $type = null) {
    global $ID;
    global $REV;
    global $lang;
    global $INPUT;
    ...
    $r_nav = '';
    if (!$text) {
        list($l_nav, $r_nav) = html_diff_navigation($versions, $pagelog, $type, $l_rev, $r_rev);
    }
    /*
     * Create diff object and the formatter
     */
 ...
 */
function html_diff_navigation($versions, $pagelog, $type, $l_rev, $r_rev) {
    global $INFO, $ID;
    ...
    $l_revs = $r_revs = $versions; // array("1465354209","1465354223","1465354242","1465354284","1465354317","1465354354","1465354385", "1465354466","1465365562","1465365627","1465695025");
    ...
    foreach ($l_revs as $rev) {
        ...
        $l_revisions[$rev] = array(
            ...
            date("Y-m-d H:i:s", $rev), // . ' ' . editorinfo($info['user'], true) . ' ' . $info['sum'],
    ...
    foreach ($r_revs as $rev) {
        ...
        $r_revisions[$rev] = array(
            $rev,
            date("Y-m-d H:i:s", $rev), // dformat($info['date']) . ' ' . editorinfo($info['user'], true) . ' ' . $info['sum'],
            $rev <= $l_rev //disable?
        );
    }
    ...
    //dropdown
    //$form = new Doku_Form(array('action' => wl()));
    $form = new Doku_Form(array('action' => ""));
    $form->addHidden('id', $ID);
    ...
    $form->addElement(
            form_makeListboxField(
                    'rev2[0]', $l_revisions, $l_rev, '', '', '', array('class' => 'quickselect')
            )
    );
    $form->addElement(form_makeButton('submit', 'diff', 'Go'));
    $l_nav .= $form->getForm();
    ...
    $form->addElement(
            form_makeListboxField(
                    'rev2[1]', $r_revisions, $r_rev, '', '', '', array('class' => 'quickselect')
            )
    );
    $form->addElement(form_makeButton('submit', 'diff', 'Go'));
    $r_nav .= $form->getForm();
